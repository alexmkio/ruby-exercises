...
RSpec.describe 'Advanced Nested Collections' do
  ...
  it 'test 7' do
    ...
    store_names.each do |store|
      stores[:"#{store}"][:employees].each { |employee| employee_names << employee }
    end
  ...
  it 'test 10' do
    ...
    # olive_garden_dish_names = stores[:olive_garden][:dishes].map { |dish| dish[:name] }
    # olive_garden_dish_names.each do |dish_name|
    #   matching_dish = stores[:olive_garden][:dishes].find { |dish| dish[:name] == dish_name }
    #   olive_garden_menu["#{dish_name}"] = matching_dish
    # end
